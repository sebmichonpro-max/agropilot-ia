...
import { INGREDIENT_UNITS } from '../../lib/unit-conversions'
...
export function IngredientsTable({ orgSlug, items: initial }: IngredientsTableProps) {
  const router = useRouter()
  const [items, setItems] = useState(initial)
  const [isPending, startTransition] = useTransition()
  const [editing, setEditing] = useState<string | null>(null)
  const [editData, setEditData] = useState<{ name: string; category: string; unit: string; price: string; supplier: string }>({ name: '', category: '', unit: 'kg', price: '', supplier: '' })
  const [adding, setAdding] = useState(false)
  const [newData, setNewData] = useState({ name: '', category: '', unit: 'kg', price: '', supplier: '' })
  ...
  function handleSave(id: string) {
    startTransition(async () => {
      const res = await updateIngredient(orgSlug, id, {
        name: editData.name,
        category: editData.category || null,
        unit: editData.unit,
        price_cents: Math.round(parseFloat(editData.price) * 100),
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const res = await createIngredient(orgSlug, {
        name: newData.name,
        category: newData.category || null,
        unit: newData.unit,
        price_cents: Math.round(parseFloat(newData.price) * 100),
        supplier: newData.supplier || null,
      })
      if (res.error) { toast.error(res.error); return }
      toast.success('Ingrédient ajouté')
      setAdding(false)
      setNewData({ name: '', category: '', unit: 'kg', price: '', supplier: '' })
      router.refresh()
    })
  }
  ...
  function handleDelete(id: string, name: string) {
    ...
    startTransition(async () => {
      await deleteIngredient(orgSlug, id)
  ...
  return (
    <div className="rounded-xl border border-ap-cream-200 bg-white overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-ap-cream-200">
        <h3 className="text-sm font-medium text-ap-green-900">Ingrédients ({items.length})</h3>
        <button
          onClick={() => setAdding(true)}
          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-ap-green-900 text-ap-green-100 text-sm font-medium hover:bg-ap-green-800 transition-colors"
        >
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-ap-cream-300">
              ...
              <th className="text-left px-4 py-2 text-ap-cream-700 font-normal hidden sm:table-cell">Catégorie</th>
              <th className="text-left px-4 py-2 text-ap-cream-700 font-normal">Unité</th>
              <th className="text-right px-4 py-2 text-ap-cream-700 font-normal">Prix</th>
          ...
          <tbody>
            {adding && (
              <tr className="border-b border-ap-cream-200 bg-ap-green-50">
                ...
                <td className="px-4 py-2 hidden sm:table-cell"><input value={newData.category} onChange={(e) => setNewData({ ...newData, category: e.target.value })} className="w-full rounded border border-ap-cream-300 px-2 py-1 text-sm" placeholder="Catégorie" /></td>
            ...
            {items.map((item) => {
              ...
              if (editing === item.id) {
                return (
                  <tr key={item.id} className="border-b border-ap-cream-200 bg-ap-cream-100">
                    ...
                    <td className="px-4 py-2 hidden sm:table-cell"><input value={editData.category} onChange={(e) => setEditData({ ...editData, category: e.target.value })} className="w-full rounded border border-ap-cream-300 px-2 py-1 text-sm" /></td>
                    <td className="px-4 py-2">
              ...
              return (
                <tr key={item.id} className="border-b border-ap-cream-200 hover:bg-ap-cream-100 transition-colors cursor-pointer" onClick={() => startEdit(item)}>
                  ...
                  <td className="px-4 py-2 text-ap-cream-800 hidden sm:table-cell">{item.category ?? '—'}</td>
                  <td className="px-4 py-2 text-ap-cream-800">{item.unit}</td>
                  <td className="text-right px-4 py-2 font-medium text-ap-green-900">{formatCents(item.price_cents)}/{item.unit}</td>
