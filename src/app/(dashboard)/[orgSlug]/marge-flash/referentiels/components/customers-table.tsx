...
export function CustomersTable({ orgSlug, items: initial }: CustomersTableProps) {
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await createCustomer(orgSlug, { name: newName.trim() })
  ...
  function handleDelete(id: string, name: string) {
    ...
    startTransition(async () => {
      await deleteCustomer(orgSlug, id)
  ...
  return (
    <div className="rounded-xl border border-ap-cream-200 bg-white p-5 space-y-4">
      <h3 className="text-sm font-medium text-ap-green-900">Clients ({items.length})</h3>
      ...
      <form onSubmit={handleAdd} className="flex gap-2">
        <input
          ...
          placeholder="Nouveau client (Leclerc, Carrefour…)"
      ...
      <div className="space-y-1">
        ...
        {items.length === 0 && <p className="text-sm text-ap-cream-600 text-center py-4">Aucun client</p>}
      </div>
    </div>
  )
}
