...
export async function POST(req: Request) {
  ...
  if (body.scannerId) {
    ...
    if (!scanner) return new Response('Scanner inconnu', { status: 400 })
    ...
    return new Response('Scanner ou requête requis', { status: 400 })
  }

  const apiKey = process.env.ANTHROPIC_API_KEY
  if (!apiKey) return new Response('Configuration IA manquante', { status: 500 })
